    ...
    return(
        <form action="#" name="form" className="form" onSubmit={formik.handleSubmit}>
            ...
            <div className="form-group">
                ...
                {formik.touched.password && formik.errors.password && <small style={{color: 'red', fontWeight: 'bold'}} className="form-text">{formik.errors.password}</small>}            </div>
            <div style={{display: 'flex', justifyContent: 'space-around', alignItems: 'center'}}>
                <button type="submit" className="btn btn-dark form-btn" >Registration</button>
                <Link style={{color: '#fff' , margin: '20px 20px 0 20px'}} to="/">Back</Link>
            </div>
        </form>
    );
}